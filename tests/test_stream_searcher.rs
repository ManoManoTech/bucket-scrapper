use bucket_scrapper::search::{SearchCollector, SearchConfig, StreamSearcher};
use std::io::Cursor;

/// Minimal collector for tests — just counts matches.
...
fn test_regex_matcher_cached() {
    ...
    let mut c1 = CountingCollector(0);
    searcher
        .search_stream("b", "k1", Cursor::new(b"Line 1\nERROR: bad\nLine 3"), &mut c1)
        .unwrap();

    let mut c2 = CountingCollector(0);
    searcher
