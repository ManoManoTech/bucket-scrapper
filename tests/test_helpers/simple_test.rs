...
use super::{TestEnvironment, TestConstants};
...
pub fn assert_success(result: &ConsolidationResult) {
    assert!(result.ok, "Consolidation should succeed. Message: {}", result.message);
    assert!(!result.uploaded_result_key.is_empty(), "Result should be uploaded to S3");
}

pub fn assert_failure(result: &ConsolidationResult) {
    assert!(!result.ok, "Consolidation should fail. Message: {}", result.message);
}